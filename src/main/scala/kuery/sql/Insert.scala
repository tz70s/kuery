...
trait InsertService {
   ...
   */
  implicit def insertHospital(hospital: Hospital): Route = complete(hospital.toString)
   ...
   */
  implicit def insertPersonnel(personnel: Personnel): Route = complete(personnel.toString)
   ...
   * TODO: We may consider to support automatically match this by opening two sql statements transactionally.
   */
  implicit def insertPharmacy(pharmacy: Pharmacy): Route = complete(pharmacy.toString)
}
